...
def listMovies(book_catalog):
    title_list = list(book_catalog.keys())
    title_list.sort()
    col = "10"
    for book in title_list:
        print()
        print(f"{'Title:':{col}} {book}")
        print(f"{'Author:':{col}} {book_catalog.get(book).get('author')}")
        print(f"{'Pub year:':{col}} {book_catalog.get(book).get('pubyear')}")
